...
static void	err_handling_in_redirect(char *redirect_token_str,
	char *expanded_str, t_status status, t_list *vars_list[3])
{
	...
	{
		redirect_symbol_ptr = ft_strchr(redirect_token_str, '>');
	...
	t_token *tokens, int i, t_list **fds_list, t_list *vars_list[3])
{
	...
	if (expand_word_token(
			tokens[i + 1], vars_list, flag, &expand_list) == E_SYSTEM)
		return (E_SYSTEM);
	if (expand_list == NULL || ft_lstsize(expand_list) >= 2)
	{
	...
	if (status != SUCCESS)
		err_handling_in_redirect(
			tokens[i].str, expand_list->content, status, vars_list);
	ft_lstclear(&expand_list, free);
	return (status);
}
